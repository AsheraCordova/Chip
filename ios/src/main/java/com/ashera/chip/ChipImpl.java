...
public java.util.Map<String, Object> getOnClickEventObj(View v) {
    ...
    public void setVisible(boolean b) {
		...
		//end - body
    @com.google.j2objc.annotations.WeakOuter
	private final class RippleTouchListener implements View.OnClickListener {
		@Override
		public void onClick(View v) {
			View view = (View) parentLL.asWidget();
			Object color = rippleColor;
	        if (color instanceof ColorStateList) {
				color = ((ColorStateList) color).getColorForState(view.getDrawableState(), r.android.graphics.Color.BLACK);
	        }
			rippleWithView(parentLL.asNativeWidget(), Math.max(view.getWidth(), view.getHeight()), ViewImpl.getColor(color));
		}
	}
	...
	private void addRippleEffect() {
		chip.setRippleInternalClickListener(new RippleTouchListener());
	}
	
	 private native void rippleWithView(Object uiview,  float radius, Object colorTo)/*-[

...
public java.util.Map<String, Object> getOnClickEventObj(View v) {
    ...
    public void setVisible(boolean b) {
		...
		//end - body
    private final static String WIDGET_TEXTVIEW = "TextView";
    private HTMLElement ripple;
	public interface HtmlClickEvent extends org.teavm.jso.dom.events.Event {
	...
	private void addRippleEffect() {
		HTMLElement chipLayout = (HTMLElement) parentLL.asNativeWidget();
		chipLayout.addEventListener("click", new org.teavm.jso.dom.events.EventListener<HtmlClickEvent>() {
			@Override
			public void handleEvent(HtmlClickEvent evt) {
				HTMLElement circleWrapper = org.teavm.jso.dom.html.HTMLDocument.current().createElement("div");
				circleWrapper.getStyle().setProperty("position", "relative");
				chipLayout.getStyle().setProperty("overflow", "hidden");
				chipLayout.getStyle().setProperty("transition", "background 400ms");
				HTMLElement circle = org.teavm.jso.dom.html.HTMLDocument.current().createElement("span");
			  	  int diameter = Math.max(((View)parentLL.asWidget()).getHeight(), ((View)parentLL.asWidget()).getWidth());
			  	  int radius = diameter / 2;
			  	  
			  	  circle.getStyle().setProperty("position", "absolute");
			  	  circle.getStyle().setProperty("border-radius", "50%");
                  ...
                  if (color instanceof ColorStateList) {
			  	  ...
			  	  circle.getStyle().setProperty("background-color", (String) ViewImpl.getColor(color));
			  	  circle.getStyle().setProperty("width", diameter + "px");
			  	  circle.getStyle().setProperty("height", diameter + "px");
			  	  circle.getStyle().setProperty("left", evt.getClientX() - chipLayout.getOffsetLeft() - radius + "px");
			  	  circle.getStyle().setProperty("top", evt.getClientY() - chipLayout.getOffsetTop() - radius + "px");
			  	  circle.setClassName("ripple");
			  	

			  	  if (ripple != null) {
			  	    ripple.getParentNode().removeChild(ripple);
			  	  }
			  	  ripple = circleWrapper;
			  	  
			  	  circleWrapper.appendChild(circle);
			  	  chipLayout.appendChild(circleWrapper);		
